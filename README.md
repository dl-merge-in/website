...
├─ HANDOVER.md         # LP運用・未完了作業・公開確認の引き継ぎ
└─ CNAME               # GitHub Pagesの独自ドメイン設定
```

## 引き継ぎと優先順位

[HANDOVER.md](HANDOVER.md) に、公開状態、正規URL、変更してはいけない設定、LP固有の残タスク、公開前チェックリストをまとめています。LPを更新する前に確認してください。アプリ本体の実装やインストーラーの詳細は、private リポジトリ側の引き継ぎ文書で管理します。
